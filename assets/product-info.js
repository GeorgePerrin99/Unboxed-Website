if (!customElements.get('product-info')) {
  ...
  // Keep pills fixed: DO NOT swap/transition the whole variant-selects element
  // Just update the selected variant JSON inside it so other logic stays correct.
  const source =
    html.querySelector('variant-selects [data-selected-variant]')?.innerHTML ||
    html.querySelector('variant-hover [data-selected-variant]')?.innerHTML;

  const destination =
    this.variantSelectors?.querySelector('[data-selected-variant]') ||
    this.querySelector('variant-hover [data-selected-variant]');
